...
let saveCheckData = () => {
    ...
    els.forEach(el => {
        ...
        var age = 1;  // TODO
        var klass = el.querySelector("td.name small").innerHTML; // Om denna inte finns?
...
let importData = () => {
    ...
    if (params.has("data")) {
        ...
        let dataStr = safeDecode(params.get("data"));
        let data = JSON.parse(dataStr);
        console.log("Imported data: ", data);

        // Convert to workable object
        let currentPersons = []
        data.persons.forEach((str) => {
            // Expected format: '<person>;<class>;<here or not>;<out running or not>'
            let p = str.split(";");
            let person = {}
            person.name = p[0];
            person.class = p[1];
        ...
        // Init page
        // TODO: Improve later
        //state._updateState(VIEWS.OVERVIEW, currentPersons);
        //let url = console.log(url.protocol + "//" + url.host + url.pathname)
        //let newUrl = new URL(url.protocol + "//" + url.host + url.pathname);
        //document.location = newUrl.href;
