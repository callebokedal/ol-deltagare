...
// Actually remove list (after confirmation)
...
let state = {
    ...
    _updateState: (view, data) => {
        state._view = view
        state._data = data
        localStorage.setItem(state_key, state._view)
        localStorage.setItem(storage_key, state._data)
    },
    ...
    //_loadData: () => {
    //	let decoded = safeDecode(localStorage.getItem(storage_key))
    //},
    loadAndShowState: () => {
        // [{"name":"Eric","class":"","status":"unknown","run":"unknown"},{"name":"Olle","class":"","status":"unknown","run":"unknown"}]
        if(state.isStateSaved()) {
            state._view = safe(localStorage.getItem(state_key))
            state._data = JSON.parse( localStorage.getItem(storage_key) )
