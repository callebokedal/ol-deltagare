...
let saveCheckData = () => {
    ...
    els.forEach(el => {
        ...
        console.log("hideBasedOnFilter result: ", hidden);
        obj.hidden = hidden;
        persons.push(obj);
    });
...
let getFilters = () => {
    return _.defaults(JSON.parse(localStorage.getItem(_storage_prefix+"filter")), {'here':true, 'nothere':true, 'out':true, 'home':true});
}
