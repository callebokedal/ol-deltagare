...
let importData = () => {
    ...
    if (params.has("data") && params.has("classes")) {
        ...
        //storage_key = _data_prefix+safeDecode(params.get("k").substring(1));
        storage_key = _data_prefix+safeDecode(params.get("k"));
        // Handle iOS bug formatting this to a phone number element
        if(storage_key.includes("href")) {
            try {
                /**
                 * Problem on IOS device
                 * 
                 * safeDecode("DwQwBAFgTgpgZgXgEQBcYBsBcAmAjABgFZcAOAFjIHYBOagZl2yQD48jSKb7HgB6EZkA");
                 * -> "<a href=\"tel:21051844799312\">21051844799312</a>"
                 */
                storage_key = _data_prefix+storage_key.match(/(tel:)(\d+)/)[2];
            } catch(err) {
                console.log("Error importing list name");
            }
        }
        ...
        alert("Lista importerad som '" + storageKeyToName(storage_key) + "'");
    } else {
...
let renderStoredLists = () => {
    ...
    for (var i = 0; i < localStorage.length; i++){
        if(localStorage.key(i).startsWith(_data_prefix)) {
            ...
            //console.log("renderStoredLists key: ", localStorage.key(i));
            console.log("renderStoredLists name: ", storageKeyToName(localStorage.key(i)));
            //document.getElementById("debug").innerHTML
            //$("#debug").append("<p>" + storageKeyToName(localStorage.key(i)) + "</p>");
        }
    }
...
let storageKeyToName = (key) => {
    //console.log("storageKeyToName: ", key);
    let d = new Date();
    d.setHours(0,0,0,0);
    if (key && key.startsWith(_data_prefix)) {
        // Assume valid key
        ...
        // Assume old key
        return "Närvaro (" + d.toLocaleTimeString() + ")";
    }
} 
...
let cloneCurrentList = (key) => {
    ...
    // Add visual feedback
    //$("#listInfo").append('<p class="text-success">Lista "' + storageKeyToName(newKey) + '" skapad</p>');
    alert("Lista '" + storageKeyToName(newKey) + "' skapad");
...
let debug = () => {
    $("#debug").html(escapeHtml($("#storedList").html()));
    //console.log("debug", escapeHtml($("#storedList").html()));
    debugModal.show();
    return false;
}
...
let debugModal = new bootstrap.Modal(document.getElementById('debugModal'), {
    keyboard: true,
    focus: true
});